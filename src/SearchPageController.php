...
class SearchPageController extends PageController {
    ...
    public function PerformSearch(){
        ...
        if ($currentSubsiteID !== null) { // Check for null instead of falsy
            self::$filters['subsite'] = (int)$currentSubsiteID;
        } else {
            self::$filters['subsite'] = 0; // Default to main site if no subsite is set
        }
        ...
        $titleMatches = ArrayList::create();
        $otherResults = ArrayList::create();

        // Loop through all the record types
        foreach ($types as $type){
            ...
            // Convert SQL results into SilverStripe objects of the appropriate class
            if ($resultIDs){
                $resultObjects = $type['ClassName']::get()->filter(['ID' => $resultIDs]);
                foreach($resultObjects as $object){
                    // Extract snippet
                    $snippet = $this->getSnippet($object, self::get_query());
                    $object->Snippet = $snippet; // Add a new property to hold the snippet

                    // Prioritize results where the title contains the query
                    if (stripos($object->Title, self::get_query()) !== false){
                        $titleMatches->push($object);
                    } else {
                        $otherResults->push($object);
                    }
                }
            }
        }

        // Merge title matches first, then other results
        $allResults->merge($titleMatches);
        $allResults->merge($otherResults);

        // Apply sorting
        $sort = false;
        // Sorting applied through form submission
    ...
    /**
     * Extract a snippet containing the search term with the term in bold.
     *
     * @param SiteTree $object
     * @param string $query
     * @return string
     */
    private function getSnippet($object, $query) {

        if (!$query) {
            return '';
        }

        // Assuming 'ElementalSearchContent' contains the searchable content
        $content = $object->ElementalSearchContent;
        ...
        // Define the length of the snippet
        $snippetLength = 150;

        // Extract a portion of the content around the search term
        $start = max($pos - ($snippetLength / 2), 0);
        $snippet = substr($content, $start, $snippetLength);

        // Ensure the snippet starts and ends at word boundaries
        $snippet = preg_replace('/^[^\s]*/', '', $snippet);
        $snippet = preg_replace('/[^\s]*$/', '', $snippet);

        // Highlight the search term in bold
        $highlighted = '<strong>' . htmlspecialchars($query ?? '') . '</strong>';
        $snippet = preg_replace("/(" . preg_quote($query ?? '', '/') . ")/i", $highlighted, $snippet);
